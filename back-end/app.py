...
def get_shop_stats():
    if not has_request_context():
        return {
            "cart": [],
            "cart_summary": {"items_count": 0, "total_price": 0},
            "shop_orders": [],
            "orders_count": 0,
            "total_spent": 0,
...
def inject_global_cart_data():
    ...
    return {
        "global_cart_count": cart_summary.get("items_count", 0),
        "global_cart_total": cart_summary.get("total_price", 0)
    }
...
def seed_challenges():
    if Challenge.query.count() == 0:
        challenges = [
            ...
            Challenge(
                title="تحدي متقدم",
                description="اطبع الأرقام من 1 إلى 50 واستبدل المضاعفات",
                language="python",
                difficulty="advanced",
                sample_input="لا يوجد",
                sample_output="1 2 3 ...",
            ...
            Challenge(
                ...
                description="احسب factorial لرقم باستخدام recursion",
                language="python",
                difficulty="advanced",
                sample_input="5",
                sample_output="120",
                starter_code="# اكتب الكود باستخدام recursion",
                points=35
            ),
            Challenge(
                title="Anagram Checker",
                description="تحقق إذا كانت كلمتين هما anagram (نفس الحروف بترتيب مختلف)",
                language="python",
                difficulty="advanced",
            ...
            Challenge(
                ...
                description="اعثر على رقمين داخل list مجموعهم يساوي target",
                language="python",
                difficulty="advanced",
            ...
            Challenge(
                ...
                description="تحقق إذا كانت الأقواس في string متوازنة بشكل صحيح",
                language="python",
                difficulty="advanced",
                sample_input="()[]{}",
                sample_output="True",
...
def codelab_page():
    ...
    result = db.session.execute(text("SELECT * FROM challenges"))
    challenges = [dict(row._mapping) for row in result]
    return render_template('codelab.html', challenges=challenges)
